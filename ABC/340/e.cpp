...
int main() {
    ...
    // range add, range max
    LazyPropagationSegmentTree<long long, long long> st(
        data,
    ...
    for (int i = 0; i < m; ++i) {
        int t;
        cin >> t;
        long long x = st.get(t);
        long long f = x / (long long)n;
        long long _m = x % (long long)n;

        // dbgcout << "t: " << t << ", x: " << x << ", f: " << f << ", _m: " << _m << endl;
        ...
        if (t + _m + 1 < n) {
            st.apply(t + 1, t + _m + 1, 1ll);
        }
        else {
            st.apply(t + 1, n, 1ll);
            st.apply(0, _m + 1 - (n - t), 1ll);
        }
