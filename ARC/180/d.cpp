...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (max_value.index == left) {
            ...
            edge_min min_value = st_min.query(left, max_value.index);
            edge_min min_value2 = st_min.query(max_value.index + 1, right + 1);
            edge second_max = st.query(min_value2.min_index, right + 1);
            cout << max_value.max + min_value.min + second_max.max << endl;
        }
